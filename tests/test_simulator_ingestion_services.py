...
@pytest.mark.asyncio
async def test_simulator_usage_skips_duplicate_untrusted_device_policy_alert():
    from app.models.alert import Alert
    from app.models.usage_record import UsageRecord
    from app.services.usage_ingestion.simulator_usage_service import (
        run_simulator_usage_ingestion_for_router,
    ...
    class FakeDb:
        ...
        async def execute(self, stmt):
            ...
            if self.execute_calls == 1:
                return FakeScalarResult(self.router)

            if self.execute_calls == 2:
                return FakeDeviceResult(self.devices)

            return FakeScalarResult(uuid4())
    ...
    assert result.records_created == 0
    assert result.blocked_devices == 1
    assert result.policy_alerts_created == 0
    assert usage_records == []
