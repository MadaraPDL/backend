...
@router.get(
    "/{router_id}/capabilities",
    response_model=MyRouterCapabilitiesResponse,
)
async def get_my_router_capabilities_endpoint(
    router_id: UUID,
    db: AsyncSession = Depends(get_db),
    current_user: AppUser = Depends(get_current_app_user),
) -> MyRouterCapabilitiesResponse:
    ...
    return get_my_router_capabilities(router)
