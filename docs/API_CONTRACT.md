...
- Ã¢â‚¬Å“Send code to emailÃ¢â‚¬Â appears only when Email MFA is active.
- Ã¢â‚¬Å“Use authenticator appÃ¢â‚¬Â appears only when Authenticator MFA is active and not already the active challenge.
- Ã¢â‚¬Å“Use backup codeÃ¢â‚¬Â appears only when `backup_codes_available=true`.
